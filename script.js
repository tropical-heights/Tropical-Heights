...
function ajouterBoutonExport() {
    const panelAdmin = document.getElementById('panel-suivi-employes');
    if (!panelAdmin) return;

    let btnExport = document.getElementById('btn-export-csv');
    if (!btnExport) {
        btnExport = document.createElement('button');
        btnExport.id = 'btn-export-csv';
        btnExport.className = 'btn-submit';
        btnExport.style.marginTop = '15px';
        btnExport.style.background = 'linear-gradient(90deg, #ff7675 0%, #ff9f43 100%)';
        btnExport.textContent = '📥 Exporter le bilan (CSV)';
        btnExport.onclick = exporterComptaCSV;
        
        // On le place juste à côté ou en dessous de ton tableau
        const tableau = document.querySelector('table');
        if (tableau) tableau.parentNode.appendChild(btnExport);
    }
}

function exporterComptaCSV() {
    const fichesCompta = JSON.parse(localStorage.getItem('fichesCompta')) || {};
    if (Object.keys(fichesCompta).length === 0) {
        alert("Aucune donnée à exporter !");
        return;
    }

    let csvContent = "data:text/csv;charset=utf-8,Collaborateur,Ventes,Total Genere,Total Benefices\n";
    
    Object.keys(fichesCompta).forEach(employe => {
        const data = fichesCompta[employe];
        csvContent += `${employe},${data.ventes},${data.ca},${data.benefices || 0}\n`;
    });

    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `compta_lucky_plucker_${new Date().toLocaleDateString().replace(/\//g, '-')}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
}

// À appeler dans ton DOMContentLoaded à la fin du fichier :
// ajouterBoutonExport();
